...
def convolution(img, kernel):
    ...
    out = np.zeros((h_img-2, w_img-2), dtype=np.float64)  # Menggunakan np.float64
...
def edge_detection():
    ...
    # Menggunakan tipe data int32 untuk menyimpan hasil convolution
    new_arr = convolution(img_arr.astype(np.int32), kernel)
...
def blur():
    ...
    # Menggunakan tipe data float64 untuk menyimpan hasil convolution
    new_arr = convolution(img_arr.astype(np.float64), kernel)
    # Mengonversi kembali ke uint8
    new_arr = np.clip(new_arr, 0, 255).astype(np.uint8)
    new_img = Image.fromarray(new_arr)
    new_img.save("static/img/img_now.jpg")
...
def binary_image(threshold_value):
    ...
    img_arr[img_arr <= threshold_value] = 0  # Piksel dengan nilai di bawah threshold menjadi hitam
    img_arr[img_arr > threshold_value] = 255  # Piksel dengan nilai di atas threshold menjadi putih
    new_img = Image.fromarray(img_arr)
    new_img.save("static/img/img_now.jpg")

def dilation():
    # Konversi gambar menjadi citra biner dengan threshold tertentu
    binary_image(95)  # Misalnya, menggunakan threshold 128
    # Setelah citra biner dibuat, lakukan dilasi
    img = Image.open("static/img/img_now.jpg")
    img_arr = np.asarray(img)
    kernel = np.array([[1, 1, 1], [1, 1, 1], [1, 1, 1]])  # Kernel untuk dilasi
    dilated_img = cv2.dilate(img_arr, kernel, iterations=1)
...
def closing():
    # Konversi gambar menjadi citra biner dengan threshold tertentu
    binary_image(95)  # Misalnya, menggunakan threshold 128
    # Setelah citra biner dibuat, lakukan closing
    img = Image.open("static/img/img_now.jpg")
    img_arr = np.asarray(img)
    kernel = np.array([[1, 1, 1], [1, 1, 1], [1, 1, 1]])  # Kernel untuk dilasi
    closed_img = cv2.morphologyEx(img_arr, cv2.MORPH_CLOSE, kernel)
...
def opening():
    # Konversi gambar menjadi citra biner dengan threshold tertentu
    binary_image(95)  # Misalnya, menggunakan threshold 128
    # Setelah citra biner dibuat, lakukan opening
    img = Image.open("static/img/img_now.jpg")
    img_arr = np.asarray(img)
    kernel = np.array([[1, 1, 1], [1, 1, 1], [1, 1, 1]])  # Kernel untuk dilasi
    opened_img = cv2.morphologyEx(img_arr, cv2.MORPH_OPEN, kernel)
...
def erosion():
    # Konversi gambar menjadi citra biner dengan threshold tertentu
    binary_image(95)  # Misalnya, menggunakan threshold 128
    # Setelah citra biner dibuat, lakukan erosi
    img = Image.open("static/img/img_now.jpg")
    img_arr = np.asarray(img)
    kernel = np.ones((3, 3), np.uint8)  # Define the erosion kernel
    eroded_img = cv2.erode(img_arr, kernel, iterations=1)
...
def count_objects():
    ...
    # Temukan kontur objek dalam citra biner
    contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    
    # Hitung jumlah kontur (objek)
